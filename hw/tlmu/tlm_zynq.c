...
static qemu_irq* zynq_get_irqs(FDTMachineInfo *fdti)
{
  ...
  idx = 29;  /* interrupts 61:68 and 84:91 */ 
  for(i = 0; i < 16; i++) {
    zynq_irq[i] = qdev_get_gpio_in(intc, idx);
    if(++idx == 37) idx = 52;
  }
...
static void tlm_zynq_init(QEMUMachineInitArgs *args)
{
    ...
    tlm_zynq_binfo.fdt = fdt;
    tlm_zynq_binfo.fdt_size = fdt_size;
    ...
    tlm_zynq_binfo.nb_cpus = smp_cpus;
    ...
    tlm_map(first_cpu, 0x40000000ULL, 0x7FFFFFFFULL,
            tlm_sync_period_ns, irqs, 16, NULL);

    tlm_register_rams();


    arm_load_kernel(arm_env_get_cpu(first_cpu), &tlm_zynq_binfo);
